...
def wrap_grouped_agg_arrow_iter_udf(f, args_offsets, kwargs_offsets, return_type, runner_conf):
    ...
    def wrapped(*col_iters):
        ...
        print(
            f"[WORKER wrap_grouped_agg_arrow_iter_udf] Called with {len(col_iters)} column iterators",
            file=sys.stderr,
            flush=True,
        )
        ...
        if len(col_iters) == 1:
            ...
            print(f"[WORKER] Single column mode", file=sys.stderr, flush=True)
            result = func(col_iters[0])
            print(f"[WORKER] Result: {result}", file=sys.stderr, flush=True)
        else:
            # Multiple columns: Iterator[Tuple[pa.Array, ...]] -> Any
            print(
                f"[WORKER] Multiple columns mode ({len(col_iters)} columns)",
                file=sys.stderr,
                flush=True,
            )
