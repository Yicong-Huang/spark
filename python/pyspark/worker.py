...
def read_udfs(pickleSer, infile, eval_type, runner_conf):
    ...
    if (
        ...
        # Convert to iterator of batches/series:
        # - Iterator[pa.Array] or Iterator[pd.Series] for single column
        # - Iterator[Tuple[...]] for multiple columns
        def mapper(batch_iter):
            if len(arg_offsets) == 1:
                adapted_iter = (batch[arg_offsets[0]] for batch in batch_iter)
            else:
                adapted_iter = (tuple(batch[o] for o in arg_offsets) for batch in batch_iter)
            return f(adapted_iter)
