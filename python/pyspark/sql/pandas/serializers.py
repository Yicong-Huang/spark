    ...
    def load_stream(self, stream):
        ...
        while dataframes_in_group is None or dataframes_in_group > 0:
            ...
            if dataframes_in_group == 1:
                ...
                print(
                    f"[SERIALIZER] Loaded {len(batches)} batches for group",
                    file=sys.stderr,
                    flush=True,
                )
                ...
                if len(batches) > 0:
                    ...
                    print(
                        f"[SERIALIZER] Number of columns: {num_cols}", file=sys.stderr, flush=True
                    )
                    ...
                    class ColumnIterators:
                        ...
                        def __getitem__(self, col_idx):
                            print(
                                f"[SERIALIZER] Getting iterator for column {col_idx}",
                                file=sys.stderr,
                                flush=True,
                            )
                    ...
                    # Empty group
                    print(f"[SERIALIZER] Empty group", file=sys.stderr, flush=True)
                    yield []
